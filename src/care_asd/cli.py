...
@fp_naa_app.command("layerwise-preflight-dev")
def fp_naa_layerwise_preflight_dev(
    base_cache_dir: Annotated[Path, typer.Option("--base-cache-dir")],
    audio_root: Annotated[Path, typer.Option("--audio-root")],
    cache_dir: Annotated[Path, typer.Option("--cache-dir")],
    output_dir: Annotated[Path, typer.Option("--output-dir")],
    ...
    """Run the bounded normal-only V8 layerwise mechanism preflight."""
    try:
        from care_asd.evaluation.fp_naa_layerwise_preflight import (
            run_layerwise_mechanism_preflight,
        )

        result = run_layerwise_mechanism_preflight(
            base_cache_directory=base_cache_dir,
            audio_root=audio_root,
            cache_directory=cache_dir,
            output_directory=output_dir,
        ...
        console.print(f"[red]FP-NAA V8 layerwise preflight failed:[/red] {exc}")
        raise typer.Exit(code=1) from exc
    status = "passed" if result.gate_passed else "failed"
    console.print(
        f"[green]FP-NAA V8 layerwise preflight complete.[/green] gate={status} "
        f"summary={result.summary_path}"
    )
