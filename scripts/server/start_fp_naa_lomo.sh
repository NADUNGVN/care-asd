...
RUN_ID="server02_fp_naa_lomo_$(date -u +%Y%m%dT%H%M%SZ)"; JOB_DIR="outputs/fp_naa/$RUN_ID"; mkdir -p "$JOB_DIR"; printf '%s\n' "$RUN_ID" > outputs/fp_naa/latest_lomo_run_id.txt
nohup setsid env -u LD_LIBRARY_PATH -u LD_PRELOAD bash scripts/server/run_fp_naa_lomo.sh "$RUN_ID" > "$JOB_DIR/launcher.log" 2>&1 < /dev/null &
printf 'FP-NAA LOMO started: run_id=%s pid=%s workers=%s\n' "$RUN_ID" "$!" "${CARE_ASD_WORKERS:-12}"
