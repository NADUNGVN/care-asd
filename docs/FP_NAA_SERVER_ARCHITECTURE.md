...
augmentation caches are reusable because V5 does not change their frontend or augmentation
contracts. For screening, V5 reuses the registered V3 C1 report artifacts after signature and
SHA-256 validation, and initializes each C2 seed from the exact matching V3 C1 checkpoint. The
checkpoint candidate, seed, compatible C1 contract, and SHA-256 are verified and written into the
new report. A mismatch causes a hard failure rather than silent retraining or fallback.

The public `fp-naa runtime-check` retains the historical tail-loss, gradient, V3 projection, and
V4 equivariance probes and additionally verifies the V5 transport/anchor objective on CUDA, so
server validation remains one short command.
